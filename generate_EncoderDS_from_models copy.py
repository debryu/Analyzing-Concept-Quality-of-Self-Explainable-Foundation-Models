...
def evaluate(model, loader, loss, version, beta, args, loc='test'):
    ...
    all_encoder = np.concatenate(all_encoder)

    stats_dict = {}
    stats_dict['concept_loss'] = concept_loss
    ...
    stats_dict['encoder'] = all_encoder
  ...
  for version in checkpoints:
    ...
    for ckpt in checkpoints[version]:
      ...
      if seed not in []:
        ...
        encoders = torch.tensor(stats_dict['encoder'])
        ...
        # Save the leakage dataset
        torch.save(leakage_dataset, os.path.dirname(os.path.realpath(__file__)) + f'/data/encoder_dataset/LD_{ver}_{seed}_{beta}.pth')
...
for version in versions:
   ...
   for check in checkpoints:
      ...
      print(f'Concept CE Loss: {res[version][check]["concept_loss"]}')
