...
def train_LF_CBM(args):
    ...
    # train_targets = torch.tensor([k['y'] for k in train_dataset.metas]).to(dtype=torch.long, device=args.device)
    # val_targets   = torch.tensor([k['y'] for k in val_dataset.metas]).to(dtype=torch.long, device=args.device)
    ...
    print(train_c[:10, :6])
    print(torch.stack([torch.from_numpy(x) for x in train_g], dim=0)[:10])
...
def get_args():
    ...
    parser.add_argument("--dataset", type=str, default="kandinsky")
    ...
    parser.add_argument("--print", type=bool, default=True, help="Print all concepts being deleted in this stage")
